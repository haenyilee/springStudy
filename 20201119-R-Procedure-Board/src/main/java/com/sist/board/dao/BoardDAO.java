...
public class BoardDAO {
	...
	public List<BoardVO> boardListData(int page)
	{
		...
		/*
		 *   CREATE OR REPLACE PROCEDURE projetBoardList(
			   pStart NUMBER,
			   pEnd NUMBER,
		...
		try
		{
			String sql="{CALL projectBoardList(?,?,?)}";
    ...
    public boolean boardUpdate(BoardVO vo)
    {
		...
		// @AfterThrowing : e.printStackTrace
		}
    	// @After : disConnection()
    	// @AfterReturning : obj
    	return bCheck;
    ...
    public int boardTotalPage()
    {
    	int total=0;
    	try {
			String sql="SELECT projectBoardTotalPage() FROM project_board";
    ...
    public List<ReplyVO> replyListData(int type,int cno,int page)
    {
       ...
       try
       {
          ...
          cs.executeQuery();
          System.out.println("replyListData프로시져");
